...
export interface FileDropMeta {
	source: FileSource
	files: File[]
	originalEvent: DragEvent | ClipboardEvent | React.ChangeEvent<HTMLInputElement>
}
...
const filesFromList = (list?: FileList | null) => (list ? Array.from(list) : [])
				...
				{children ?? (
					<div className="flex flex-col items-center gap-3">
						...
						<p className="text-muted-foreground/80 text-xs">
							Supports click, drag-and-drop, or clipboard paste
						</p>
					</div>
				)}
			</div>
			...
			{isDraggingViewport && !disabled && (
				<div className="z-50 fixed inset-0 flex justify-center items-center bg-background/90 pointer-events-none">
					...
					<div className="bg-background/80 shadow-xl px-10 py-8 border-4 border-primary/70 border-dashed rounded-2xl text-center">
						<p className="font-semibold text-primary text-base">Drop your file to upload</p>
						<p className="text-muted-foreground text-sm">Release anywhere in the window</p>
					</div>
				</div>
			)}
		</>
